...
**作用:**在$\mathcal{o}(n)$时间内得出字符串的最长回文子串
...
void solve() {
    ...
    function<int(int, bool, bool)> count = [&](int id, bool is_limit, bool is_num)->int {
        if (id == len)
            return 1;
        if (!is_limit && dp[id][1] != -1 && is_num)
            return dp[id][1];
        int res = 0;
        if (!is_num)
            res = count(id + 1, false, false);
        int up = is_limit ? s[id] - '0' : 9;
        for (int i = 1 - is_num; i <= up; i++)
        {
            if (i == k)continue;
            res += count(id + 1, is_limit && i == up, true);
        }
        if (!is_limit && is_num)
            dp[id][1] = res;
        return res;
    };
    int ans = count(0, true, false);
    ans--;
    cout << ans << '\n';
}
